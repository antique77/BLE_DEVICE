...
#endif // BOOTLOADER_SECURE
#else
// ★ CUSTOM PATCH: 应用层 linkerfile.ld 使用 ORIGIN = 0x14000
//   原始值 0x00006000UL 导致 BL 启动后去错误地址找应用 → 验证失败 → 卡在 XMODEM 模式
#define BTL_APPLICATION_BASE              (FLASH_BASE + 0x00014000UL)   // ★ 改为 80KB
#endif // BOOTLOADER_APPLOADER
#define BTL_MAIN_STAGE_MAX_SIZE           (BTL_APPLICATION_BASE \
                                           - BTL_FIRST_STAGE_SIZE)
